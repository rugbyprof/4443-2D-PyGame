...
class Player(PlayerAnimation):
    ...
    def update(self):
        ...
        self.rect.centery += config['gravity']
...
def main():
    ...
    # load our background
    background = pygame.image.load(config['backgrounds']['gravity_00'])
    walls = loadJson(config['walls']['gravity_00'])
    ...
    player = Player(path=config['sprite_sheets']['green_monster']['path'],loc=(random.randint(0,width),random.randint(0,height)),speed=10,dx=1,dy=1)

...
class PlatFormGenerator(object):
    ...
    def addToMatrix(self,block_type,width,height,startx,starty=0,tile_type=1):
        ...
        if block_type == 'platform':
            ...
            self.matrix[0][startx] ='06'        # bottom left floor
            for j in range(startx+1,endx-1):
                self.matrix[0][j] = '07'        # bottom mid floor
            self.matrix[0][endx-1] ='08'        # bottom right floor

            # middle of floor chunk
            rows = range(1,height-1)
            for i in rows:
                self.matrix[i][startx] = '03'       # mid left
                for j in range(startx+1,endx-1):
                    self.matrix[i][j] = '04'        # mid mid
                self.matrix[i][endx-1] = '05'       # mid left

            # top of floor chunk
            self.matrix[height-1][startx] ='00'         # top left floor
            for j in range(startx+1,endx-1):
                self.matrix[height-1][j] = '01'        # top mid floor
            self.matrix[height-1][endx-1] ='02'        # top right floor
